            ...
            @foreach($seccion->contenidosNavbar as $contenido)
                <div class="col-md-6 col-lg-4 mb-4">
                    <div class="p-3 rounded shadow-sm"
                         style="background: var(--azul-suave); color: var(--azul-oscuro); border-left: 5px solid var(--azul-medio);">
                        <h5 class="fw-bold mb-2">{{ $contenido->nombre }}</h5>

                        <a href="{{ route('public.navbar.contenidos.mostrar', $contenido->id) }}"
                           class="btn btn-sm"
                           style="background-color: var(--azul-medio); color: var(--blanco); border-radius: 20px;">
                            Ver contenido
                        </a>
                    </div>
                </div>
