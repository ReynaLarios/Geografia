            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($cuadro->archivo)
                            @php
                                $nombre = pathinfo($cuadro->archivo, PATHINFO_BASENAME);
                                $tamanoMB = number_format(Storage::disk('public')->size($cuadro->archivo)/1024/1024,2);
                            @endphp
                            <a href="{{ asset('storage/'.$cuadro->archivo) }}" target="_blank">{{ $nombre }}</a> ({{ $tamanoMB }} MB)
                        @endif
                        <input type="file" name="cuadros[{{ $index }}][archivo]" class="form-control mt-1">
                    </td>
