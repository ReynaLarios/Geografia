...
<main class="p-4" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">
    ...
    <form action="{{ route('contenidos.actualizar', $contenido->slug) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select name="seccion_id" class="form-select" required>
                @foreach($secciones as $sec)
                    <option value="{{ $sec->id }}"
                        @selected(old('seccion_id') == $sec->id || (old('seccion_id') === null && $contenido->seccion_id == $sec->id))>
                        {{ $sec->nombre }}
                    </option>
                @endforeach
            </select>
